...
extended_db_file = [item for item in os.listdir(extended_db_dir) if item.endswith('_covering_' + percent_spectra + '_percent_ribP_elonF_spectra.tsv.0.01.tsv')][0]
...
with open(out_dir + 'unique_peptides/'+mgf_fname+'_covering_'+str(percent_spectra)+'_percent_ribP_elonF_spectra_extended.fasta', 'w') as out_f:
    for i, peptide in enumerate(extended_db_peptides):
        out_f.write('>peptide'+str(i+1)+'\n')
        out_f.write(peptide+'\n')

with open(out_dir + 'unique_peptides/'+mgf_fname+'_all_peptides_covering_'+str(percent_spectra)+'_percent_ribP_elonF_spectra.fasta', 'w') as out_f:
