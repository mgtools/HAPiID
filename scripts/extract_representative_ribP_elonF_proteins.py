...
for genome in remaining_HEG_list:
    if 'UMGS' in genome:
        copyfile(umgs_dir+genome+'_ribP_elonF_proteinSeqs.faa', representative_ribP_elonF_dir + genome+'_ribP_elonF_proteinSeqs.faa')
    elif 'GCF_' in genome:
        fname = [item for item in os.listdir(ncbi_dir) if item.startswith(genome) and item.endswith('_ribP_elonF_proteinSeqs.faa')]
        if len(fname) > 1:
            print('more than one: '+genome)
        else:
            copyfile(ncbi_dir+fname[0], representative_ribP_elonF_dir + fname[0])
    else:
        copyfile(hbc_dir+genome+'_ribP_elonF_proteinSeqs.faa', representative_ribP_elonF_dir + genome+'_ribP_elonF_proteinSeqs.faa')
